...
document.getElementById("uploadForm").addEventListener("submit", function(event) {
    ...
    if (filesToUpload.length === 0) {
      // No file selected
      document.getElementById("message").innerHTML = "<span style='color: red; font-weight: bold;'>Please select a file to upload</span>";
      return;
    }
...
function uploadFilesSequentially(filesToUpload, index) {
    if (index >= filesToUpload.length) {
      document.getElementById("message").innerHTML = "<span style='color: green; font-weight: bold;'>All files uploaded successfully!</span>";
      return;
    }
    ...
    xhr.onload = function () {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        document.getElementById("message").innerHTML = "<span style='color: green; font-weight: bold;'>" + xhr.responseText + "</span>";
        uploadFilesSequentially(filesToUpload, index + 1); // Upload next file
      } else {
        // Request failed
        document.getElementById("message").innerHTML = "<span style='color: red; font-weight: bold;'>Error uploading file</span>";
      }
    };
    
    xhr.onerror = function () {
      // Network errors
      document.getElementById("message").innerHTML = "<span style='color: red; font-weight: bold;'>Network error occurred while uploading file</span>";
    };
